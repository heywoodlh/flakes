...
files=("zen.macos-x64.dmg" "zen.macos-aarch64.dmg" "zen.linux-specific.tar.bz2")
...
do
  ...
  [[ "$file" == "zen.linux-specific.tar.bz2" ]] && sed -Ei "s/x86_64-linux-hash = \"(.*)\"/x86_64-linux-hash = \"${myHash}\"/g" zen.nix
done
